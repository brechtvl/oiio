...
ColorConfig::Impl::inventory()
{
    ...
    if (config_ && !disable_ocio) {
        try {
            bool nonraw = false;
            // Include inactive color spaces. In older ACES configs (like
            // v2.1.0 bundled with OpenColorIO 2.3) the display color spaces
            // are inactive, but we rely on them for interop IDs.
